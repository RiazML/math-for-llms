...
> **CRITICAL — TOKEN GENERATION LIMIT RULE**
>
> The API has a maximum token generation limit per response. **Every chunk of written content must not exceed ~300 words.** This applies to ALL file writing: `notes.md`, Python builder scripts, and any other output. Violating this causes the response to be cut off mid-write, producing broken files.
>
> **Rules:**
> - Never write more than ~300 words of file content in a single tool call
> - For `notes.md`: use `Write` for the first chunk, then `Bash cat >>` heredoc for each subsequent chunk
> - For Python builder scripts: split into multiple scripts if the script body exceeds ~300 words — write part 1 to `/tmp/build_theory_p1.py`, run it; write part 2 to `/tmp/build_theory_p2.py` (append pattern), run it; etc.
> - Always complete a chunk fully before starting the next one
> - Never attempt to write an entire file in one shot — always plan the chunk boundaries before writing

### Chunk plan for each file

**`notes.md`** — split by section, ~300 words per `cat >>` call:

- Chunk 1 (`Write`): header, overview, prerequisites, companion notebooks, learning objectives, full TOC
- Chunk 2 (`cat >>`): Section 1 (Intuition) + Section 2 (Formal Definitions)
- Chunk 3 (`cat >>`): Section 3 (first half of Core Theory)
- Chunk 4 (`cat >>`): Section 3 (second half) + Section 4
- Chunk 5 (`cat >>`): Sections 5–6
- Chunk 6 (`cat >>`): Sections 7–8 (Advanced Topics + Applications)
- Chunk 7 (`cat >>`): Common Mistakes table + Exercises + AI Impact table + Conceptual Bridge + ASCII diagram

**`theory.ipynb`** — split builder script by notebook section:

- Part 1 script (`/tmp/build_theory_p1.py`): cells 0–~15 (setup + sections 1–4), creates the file
- Part 2 script (`/tmp/build_theory_p2.py`): cells ~16–~30 (sections 5–8), appends to file
- Part 3 script (`/tmp/build_theory_p3.py`): cells ~31–end (sections 9–end + closing), appends to file
- Validate after each part: `python3 -c "import json; nb=json.load(open('theory.ipynb')); print(len(nb['cells']), 'cells')"`

**`exercises.ipynb`** — split builder script by exercise group:

- Part 1 script (`/tmp/build_exercises_p1.py`): header cell + setup cell + exercises 1–4, creates the file
- Part 2 script (`/tmp/build_exercises_p2.py`): exercises 5–8 + closing cell, appends to file
- Validate after each part

### Execution order with chunking

1. **Chunk 1**: `notes.md` chunk 1 — Write tool
2. **Chunk 2**: `notes.md` chunk 2 — `cat >>`
3. *(repeat for all notes.md chunks)*
4. **Chunk N**: theory part 1 builder script — Write + run
5. **Chunk N+1**: theory part 2 builder script — Write + run
6. *(repeat for all theory parts)*
7. **Chunk M**: exercises part 1 builder script — Write + run
8. **Chunk M+1**: exercises part 2 builder script — Write + run
